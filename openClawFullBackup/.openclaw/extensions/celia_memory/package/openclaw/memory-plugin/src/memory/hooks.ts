...
import type { CeliaMemoryConfig } from "../core/config.js";
...
import {
  ...
  clearSessionPromptBuffer,
  getSessionPromptBuffer,
 ...
 */
const lastFixedLoadTokens = new Map<string, number>();
 ...
 */
const lastFullSyncMs = new Map<string, number>();
...
function normalizeStateWorkspace(workspaceDir?: string): string {
...
function globalSyncStateKey(tenantId: string, userId: string): string {
 ...
 */
export function markFixedLoadDirty(tenantId: string, userId: string): void {
  dirtyKeys.add(globalSyncStateKey(tenantId || "default", userId));
}

interface MdFileSyncResult {
  guidanceSystemContext?: string;
}

function joinContextSections(
...
function renderPatchBodyForPrompt(patch: MarkerPatch): string | undefined {
...
function renderGuidanceSystemContext(
  guidancePatch: MarkerPatch,
): string | undefined {
  try {
 ...
 */
function sanitizeSessionPromptBufferLine(input: string): string {
...
function renderSessionPromptBufferBlock(
 ...
 */
export function _sanitizeSessionPromptBufferLineForTesting(
  input: string,
): string {
 ...
 */
export async function fetchAndRenderSessionPromptBuffer(
  api: OpenClawPluginApi,
  cfg: CeliaMemoryConfig,
  ctx: { sessionId?: string; userId?: string; workspaceDir?: string },
): Promise<{ appendSystemContext: string } | undefined> {
  const openclawSessionId = ctx.sessionId ?? "";
  if (!openclawSessionId) return undefined;
  ...
  const key =
    buildSessionPromptBufferKey(tenantId, userId, openclawSessionId);
  const items = getSessionPromptBuffer(key);
  const renderedText = renderSessionPromptBufferBlock(items);

  api.logger.info?.(
    `${pfx} event=done count=${items.length} `
 ...
 * 当前固定走 md_files 加载路径。
 */
export function registerMdFileSync(
  api: OpenClawPluginApi,
  ...
  async function runAllSyncs(
    stage: "session_start" | "after_compaction" | "before_prompt_build",
    ctx: { workspaceDir?: string; userId?: string },
  ): Promise<MdFileSyncResult> {
    /* 入口诊断日志：记录触发事件 + 关键 ctx 字段，便于排查
     * "hook 没跑/早期 return" 类问题。 */
    api.logger.info?.(
      `memory-celia: [mdFileSync] stage=${stage} fired ` +
    ...
    if (!ctx.workspaceDir) {
      api.logger.warn?.(
        `memory-celia: [mdFileSync] stage=${stage} skip ` +
    ...
    if (stage === "before_prompt_build") {
      ...
      if (isDirty) {
        api.logger.info?.(
          `${pfx} stage=${stage} event=dirty_refresh ` +
    ...
    try {
      ...
      api.logger.warn?.(
        `${pfx} stage=${stage} event=ensure_md_failed ` +
          `err=${String(err)}`,
      );
      return {};
    }
    ...
    const fetchOk = <T>(idx: number, label: string): T | null => {
      const r = fetched[idx];
      if (r.status === "fulfilled") return r.value as T;
      api.logger.warn?.(
        `${pfx} stage=${stage} event=fetch_failed source=${label} ` +
          `err=${String(r.reason)}`,
      );
      return null;
    };
    const l0Body  = fetchOk<string>(0, "l0");
    const allL1   = fetchOk<Awaited<ReturnType<typeof fetchL1Index>>>(
      1, "l1_index",
    );
    const scenes      = allL1
    ...
    const userPatches: MarkerPatch[] = [];
    const memoryPatches: MarkerPatch[] = [];
    if (l0Body !== null) {
      userPatches.push({
    ...
    const guidancePatch: MarkerPatch = {
    ...
    const writeJobs: {
      patches: MarkerPatch[];
      promise: Promise<MarkerWriteResult[]>;
    }[] = [];
    ...
    const guidanceSystemContext = stage === "before_prompt_build"
      ...
      : undefined;
    const writeRes = await Promise.allSettled(
    ...
    for (let i = 0; i < writeRes.length; i++) {
      const wr = writeRes[i];
      if (wr.status !== "fulfilled") continue;
    ...
    api.logger.info?.(
      `${pfx} stage=${stage} event=done dur_ms=${elapsedMs} ` +
  ...
  api.on("before_prompt_build", async (_event, ctx) => {
    ...
    const promptBuffer =
      await fetchAndRenderSessionPromptBuffer(api, cfg, ctx);
  ...
  api.logger.info?.(
    "memory-celia: [mdFileSync] enabled " +
 ...
 */
export function registerConversationCapture(
  api: OpenClawPluginApi,
  client: CeliaMcpClient,
  cfg: CeliaMemoryConfig,
): void {
  api.on("agent_end", async (event, ctx) => {
    ...
    if (!event.success || !event.messages || event.messages.length === 0) {
      api.logger.info?.(
        `memory-celia: [capture] skip — success=${event.success}, msgs=${event.messages?.length ?? 0}`,
      );
      return;
    }
    ...
    try {
      ...
      const lastUserRaw = (() => {
        const c = (lastUserMsg as Record<string, unknown>).content;
      ...
      if (/session.*(started|reset).*\/(new|reset)/i
        ...
        || /Run your Session Startup sequence/i
          .test(lastUserRaw.slice(0, 200))) {
        api.logger.info?.(
          "memory-celia: [capture] skip — session startup sequence",
        );
        return;
      }
      ...
      const sanitizeAssistantBlocks = (
        ...
        for (const block of content as Array<Record<string, unknown>>) {
          ...
          if (!KEEP_ASSISTANT_BLOCK_TYPES.has(t)) continue;
          ...
          if (t === "thinking") delete copy.thinkingSignature;
          /* toolCall 的 id 仅用于与 toolResult 配对，
           * toolResult 已整条丢弃，id 无用且浪费 token */
          if (t === "toolCall") delete copy.id;
      ...
      for (let i = lastUserIdx; i < msgs.length; i++) {
        ...
        if (role === "user") {
          ...
          const blocks = sanitizeAssistantBlocks(
          ...
          if (MEMORY_TOOLS.has(tn)) continue;
          ...
          if (typeof errContent === "string") {
            ...
            for (const b of errContent as Array<
              Record<string, unknown>
            >) {
              if (b?.type === "text" && typeof b.text === "string") {
                errText += (b.text as string) + "\n";
              }
            }
          }
      ...
      try {
        ...
        api.logger.warn?.(
          `memory-celia: ensureToolSession failed: ${String(err)}`,
        );
        return;
      }
      ...
      const hasStoreUrgentIngestValue =
        consumeStoreUrgentIngest(
          buildStoreUrgentIngestKey(tenantId, userId, conversationId),
        )
        || consumeStoreUrgentIngest(
          buildStoreUrgentIngestKey(tenantId, userId, userId),
        )
      ...
      api.logger.info?.(
        `memory-celia: [capture] last round ${cleaned.length} messages, ` +
      ...
      api.logger.info?.(
        `${logPrefix("capture", captureTraceId)} memory_add params: ${JSON.stringify(addParams)}`,
      );

      const result = await client.callTool("memory_add", addParams);
      api.logger.info(
        `memory-celia: [capture] → ${JSON.stringify(result)}`,
      );
      ...
      for (let j = 0; j < cleaned.length; j++) {
        api.logger.info?.(
          `memory-celia: [capture-cleaned] [${j}] ${JSON.stringify(cleaned[j])}`,
        );
      }
    } catch (err) {
 ...
 */
export function registerTokenStatsCapture(
  api: OpenClawPluginApi,
  client: CeliaMcpClient,
  cfg: CeliaMemoryConfig,
): void {
  api.on("agent_end", async (event, ctx) => {
    ...
    if (!client.isConnected() && !(await client.waitReady())) return;
    ...
    for (let i = lastUserIdx; i < msgs.length; i++) {
      ...
      api.logger.info?.(
        `memory-celia: [token-stats] [${i - lastUserIdx}] ` +
    ...
    for (let i = lastUserIdx + 1; i < msgs.length; i++) {
      ...
      if (!RECALL_TOOLS.has(tn)) continue;
    ...
    const fixedLoadTokens =
      lastFixedLoadTokens.get(exactFixedKey)
    ...
    try {
      ...
      api.logger.info?.(
        `memory-celia: [token-stats] round=${roundIndex} ` +
      ...
      api.logger.warn?.(
        `memory-celia: token stats report failed: ${String(err)}`,
      );
    }
  });
}
...
export function registerServedL1Cleanup(
  api: OpenClawPluginApi,
  cfg: CeliaMemoryConfig,
): void {
  api.on("session_end", (_event, ctx) => {
    ...
    try {
      ...
      clearSessionPromptBuffer(
        buildSessionPromptBufferKey(tenant, user, conv),
      );
    } catch (err) {
      api.logger.warn(
        `session cleanup failed: ${String(err)}`,
      );
    }
  });
}
  ...
  api.on("session_end", (_event, ctx) => {
    ...
    if (tenant || user || conv) {
      try {
        ...
        api.logger.warn(
          `servedL1: cleanup failed: ${String(err)}`,
        );
      }
    }
  });
 ...
 */
export function registerGatewayObserver(api: OpenClawPluginApi): void {
  api.on("llm_input", (event, ctx) => {
    const msgs = (event.historyMessages ?? []) as Array<Record<string, unknown>>;

    /* ---- 解析 system prompt ---- */
    const sys = msgs.find((m) => m?.role === "system");
    const sysText = typeof sys?.content === "string"
      ? (sys.content as string)
      : Array.isArray(sys?.content)
        ? ((sys!.content as Array<Record<string, unknown>>)
            .map((b) => (typeof b?.text === "string" ? (b.text as string) : ""))
            .join("\n"))
        : "";

    const profileMatch = sysText.match(
      /<user-profile>([\s\S]*?)<\/user-profile>/,
    );
    const hasProfile = !!profileMatch;
    const profileLen = profileMatch ? profileMatch[1].length : 0;

    /* ---- 找最近一条 progressive-loading toolResult ---- */
    let lastProgResult: string | undefined;
    let lastProgTool: string | undefined;
    for (let i = msgs.length - 1; i >= 0; i--) {
      const m = msgs[i];
      const tn = m?.toolName;
    ...
    /* ---- 汇总日志 ---- */
    api.logger.info(
      `memory-celia: [LLM_IN] sid=${ctx.sessionId} msgs=${msgs.length} ` +
    ...
    if (hasProfile && profileLen > 0) {
      api.logger.info(
        `memory-celia: [LLM_IN] user-profile head="` +
          `${profileMatch![1].slice(0, 300).replace(/\n/g, " ")}"`,
      );
    }
    if (lastProgResult) {
      api.logger.info(
        `memory-celia: [LLM_IN] ${lastProgTool} tool_result head="` +

...
import { extractL0Body, } from "./fixedContext.js";
 ...
 */
export async function fetchL0(client, tenantId, userId, ctx) {
    ...
    const raw = await Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error("L0 fetch deadline exceeded")), ms)),
    ]);
 ...
 */
export async function syncL0ToUserMd(client, workspaceDir, tenantId, userId, ctx) {
    /* ========== 阶段一：拉取 L0 ========== */
    let l0Body;
    try {
        ...
        ctx.logger?.warn?.(`memory-celia: [l0Sync] trace=${ctx.traceId} ` +

...
export function deriveCeliaLogFilePath(resolvedDbPath: string): string {
  ...
  if (
    basename(parentDir) === "memory" &&
    basename(grandParentDir) === "workspace"
  ) {
    return join(
      dirname(grandParentDir),
      "logs",
      "celia_memory",
      "celia_memory.log",
    );
...
export function ensureCeliaLogDir(logFilePath: string): void {

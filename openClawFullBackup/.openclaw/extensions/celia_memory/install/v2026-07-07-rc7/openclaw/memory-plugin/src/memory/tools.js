...
import { buildStoreUrgentIngestKey, markStoreUrgentIngest, } from "./store-signal.js";
import { markFixedLoadDirty } from "./hooks.js";
import { truncateUtf8Safe, utf8ByteLength } from "./markerProtocol.js";
import { appendSessionPromptBuffer, buildSessionPromptBufferKey, } from "./session-prompt-buffer.js";
...
const MEMORY_CLOSED_RECALL_TEXT = "由于梦境记忆开关关闭，本工具不产生召回结果，是正常现象，"
 ...
 */
function trimSearchContent(raw, maxBytes) {
    const report = {
        trimmed_count: 0,
    ...
    const trimmed = list.map((it) => {
        ...
        if (typeof hit.content !== "string")
            return it;
        if (utf8ByteLength(hit.content) <= maxBytes)
            return it;
...
function trackServedL1(tenantId, userId, conversationId, path) {
 ...
 * 淘汰兜底。
 */
export function clearServedL1ForSession(tenantId, userId, conversationId) {
    servedL1Paths.delete(buildServedL1Key(tenantId, userId, conversationId));
}
/** 获取已加载的 L1 paths（per-conversation；空集返回 undefined）。 */
function getServedL1Paths(tenantId, userId, conversationId) {
    const set = servedL1Paths.get(buildServedL1Key(tenantId, userId, conversationId));
    return set && set.size > 0 ? Array.from(set) : undefined;
}
/** 测试钩子：清空 servedL1Paths（仅供 vitest fixture）。 */
export function _resetServedL1PathsForTesting() {
    servedL1Paths.clear();
}
/** 测试钩子：track 接口直暴（仅供 vitest，不在生产路径调用）。 */
export function _trackServedL1ForTesting(tenantId, userId, conversationId, path) {
    trackServedL1(tenantId, userId, conversationId, path);
}
/** 测试钩子：query 接口直暴（仅供 vitest，不在生产路径调用）。 */
export function _getServedL1PathsForTesting(tenantId, userId, conversationId) {
 ...
 */
export function buildDedupPolicyArg(userArg, cfgPolicy) {
    if (!userArg && !cfgPolicy)
        return undefined;
    const ua = userArg ?? {};
    const cp = cfgPolicy ?? {};
    const out = {};
...
export function shouldEnableTimeHint(query, explicit) {
    if (explicit === true)
        return true;
    if (explicit === false)
        return false;
...
function buildMemoryClosedPayload(memoryState, base) {
    return {
        ...base,
...
function buildMemoryClosedDetails(memoryState, base) {
    return {
        ...base,
...
export function registerTools(api, client, cfg) {
    ...
    api.on("before_tool_call", (event, ctx) => {
        ...
        if (!ctx.sessionId && !sessionIdMissingWarned) {
            api.logger.warn?.("[celia] before_tool_call: ctx.sessionId is undefined; "
     ...
     * #2/#14 自动注入方案。
     */
    function resolveTenantUser(params) {
        const p = (params ?? {});
        const explicitUser = typeof p.user_id === "string" && p.user_id ? p.user_id : undefined;
        const injectedUser = typeof p._userId === "string" && p._userId ? p._userId : undefined;
        const user_id = explicitUser ?? injectedUser ?? cfg.userId;
        const explicitTenant = typeof p.tenant_id === "string" && p.tenant_id ? p.tenant_id : undefined;
    ...
    api.registerTool({
        name: "memory_store",
        label: "Memory Store",
        description: "Call only when: (1) the user explicitly asks you to remember durable information " +
        ...
        async execute(_toolCallId, params) {
            const { text, _userId, _conversationId } = params;
            const userId = _userId ?? cfg.userId;
            const openclawSessionId = _conversationId ?? "";
            const tenantId = cfg.tenantId ?? "default";
            if (openclawSessionId) {
                appendSessionPromptBuffer(buildSessionPromptBufferKey(tenantId, userId, openclawSessionId), text);
            }
            markStoreUrgentIngest(buildStoreUrgentIngestKey(tenantId, userId, openclawSessionId || userId));
            api.logger.info(`[memory_store] session-prompt-buffer text="${text.slice(0, 120)}" ` +
    ...
    api.registerTool({
        ...
        parameters: Type.Object({
            ...
            memoryId: Type.Optional(Type.Number({ description: "Specific memory ID to delete" })),
        }),
        async execute(_toolCallId, params) {
            ...
            const { query, memoryId, _userId } = params;
            const userId = _userId ?? cfg.userId;
            ...
            if (query) {
                ...
                const list = data.results
                    .map((r) => `- [id:${r.id}] ${(r.content ?? "").slice(0, 60)}...`)
                    .join("\n");
                return {
                    content: [
                        {
    ...
    api.registerTool({
        ...
        description: "加载场景记忆摘要原文（按 memory_scene_list_load 返回的 path）。" +
        ...
        parameters: Type.Object({
            ...
            tenant_id: Type.Optional(Type.String({ description: "租户 ID（可选；插件自动注入 cfg.tenantId 或 \"default\"）" })),
            user_id: Type.Optional(Type.String({ description: "用户 ID（可选；插件自动注入 ctx._userId 或 cfg.userId）" })),
        }),
        async execute(_toolCallId, params) {
            ...
            if (memoryState === 0) {
                ...
                api.logger.info(`${pfx} event=skip reason=memory_disabled ` +
            ...
            const traceId = genTraceId();
            const pfx = logPrefix("memory_scene_load", traceId);
            const pathsArr = Array.isArray(p.paths) && p.paths.length > 0 ? p.paths : null;
            if (pathsArr === null) {
                return {
                    content: [{
                            type: "text",
            ...
            const t0 = Date.now();
            api.logger.info(`${pfx} event=start mode=batch n=${pathsArr.length} ` +
            ...
            const memtypePaths = pathsArr.filter((pp) => typeof pp === "string" && pp.startsWith("L1_memtype_")).length;
            for (const pp of pathsArr) {
                if (typeof pp === "string") {
                    trackServedL1(tenant_id, user_id, conversationId, pp);
                }
            }
            api.logger.info(`${pfx} event=done mode=batch dur_ms=${durMs} ` +
    ...
    api.registerTool({
        ...
        description: "按向量语义检索原子事实。用于获取与 query 相关的事实细节。" +
        ...
        parameters: Type.Object({
            ...
            top_k: Type.Optional(Type.Number({ description: "返回条数，默认 5（之前 10）" })),
            is_procedural: Type.Optional(Type.Boolean({
                description: "仅查程序记忆。执行/继续/修复/调试/构建/发布任务前设 true，" +
                    "用任务/工具关键词召回流程、偏好和历史踩坑。",
            })),
            dedup_policy: Type.Optional(Type.Object({
                enable_lineage_dedup: Type.Optional(Type.Boolean({ description: "启用血缘去重（默认 true）" })),
                served_l1_decay: Type.Optional(Type.Number({
                    description: "已送达摘要来源事实降权系数（0-1，默认 0.5）",
                })),
            }, { description: "血缘去重策略" })),
            tenant_id: Type.Optional(Type.String({ description: "租户 ID（可选；插件自动注入 cfg.tenantId 或 \"default\"）" })),
            user_id: Type.Optional(Type.String({ description: "用户 ID（可选；插件自动注入 ctx._userId 或 cfg.userId）" })),
            time_hint: Type.Optional(Type.Boolean({
                description: "查询到时间范围/时间点时设 true；" +
        ...
        async execute(_toolCallId, params) {
            ...
            if ("category" in legacy || "memory_type" in legacy) {
                return {
                    content: [{
                            type: "text",
                            text: "category/memory_type filters were removed; use " +
            ...
            const { query, top_k, is_procedural, dedup_policy, time_hint, _conversationId } = params;
            const { tenant_id, user_id } = resolveTenantUser(params);
            ...
            const traceId = genTraceId();
            const pfx = logPrefix("memory_record_search", traceId);
            api.logger.info(`${pfx} event=start user=${formatScopedId("u", user_id)} ` +
            ...
            if (is_procedural === true)
                args.is_procedural = true;
            if (timeHint)
                args.time_hint = true;
            ...
            if (mergedDedup != null)
                args.dedup_policy = mergedDedup;
            /* 注入已送达 L1 路径（per-conversation；Cache P1.1） */
            const served = getServedL1Paths(tenant_id, user_id, conversationId);
            if (served)
                args.served_l1_paths = served;
            const raw = await client.callTool("memory_search_l2", args, undefined, { traceId });
            ...
            api.logger.info(`${pfx} event=done dur_ms=${durMs} body_len=${text.length} ` +
    ...
    api.registerTool({
        ...
        description: "检索原始会话中的完整上下文片段。用于回溯具体对话原文。" +
        ...
        parameters: Type.Object({
            ...
            tenant_id: Type.Optional(Type.String({ description: "租户 ID（可选；插件自动注入 cfg.tenantId 或 \"default\"）" })),
            user_id: Type.Optional(Type.String({ description: "用户 ID（可选；插件自动注入 ctx._userId 或 cfg.userId）" })),
            sessionIdFilter: Type.Optional(Type.String({ description: "数据 session 过滤条件（可选）" })),
        }),
        async execute(_toolCallId, params) {
            if (!client.isConnected() && !(await client.waitReady())) {
                return {
            ...
            const { query, top_k, sessionIdFilter } = params;
            const { tenant_id, user_id } = resolveTenantUser(params);
            const sessionId = await ensureToolSession(client, user_id);
            const traceId = genTraceId();
            ...
            const raw = await client.callTool("memory_search_l3", args, undefined, { traceId });
    ...
    api.registerTool({
        ...
        description: "获取当前用户的场景记忆索引 JSON 原文（含 path、summary、版本等元数据），" +
    ...
    function parseL0Tier(tier) {
        if (tier === 0 || tier === "edge")
            return 0;
        if (tier === 1 || tier === "cloud_s")
            return 1;
        if (tier === 2 || tier === "cloud_l")
            return 2;
        return 0;
    }
    api.registerTool({
        ...
        description: "按需获取 Celia 全局概览（用户画像 / 偏好 / 程序索引 / 前瞻）。" +
        ...
        async execute(_toolCallId, params) {
            ...
            const tier = parseL0Tier(params.tier);
            const traceId = genTraceId();
            const pfx = logPrefix("memory_get_l0_global_summary", traceId);
            api.logger.info(`${pfx} event=start user=${formatScopedId("u", user_id)} tier=${tier}`);
            const t0 = Date.now();
            /* C 端 ParseL0GlobalSummaryArgs 只识别 camelCase userId / tier,
             * 不接受 snake_case;TS wrapper 必须显式 mapping。 */
            const raw = await client.callTool("memory_get_l0_global_summary", { userId: user_id, tier, _trace_id: traceId }, undefined, { traceId });
            const durMs = Date.now() - t0;
            const text = typeof raw === "string" ? raw : JSON.stringify(raw);
            /* DFX:size_bytes 用于评估 subagent L0 token 成本是否随 tier 合理收紧。 */
            api.logger.info(`${pfx} event=done dur_ms=${durMs} body_len=${text.length} tier=${tier} ` +
    ...
    api.registerTool({
        name: "memory_flush",
        label: "Memory Flush",
        description: "Synchronously drain the async ingest queue and force-run periodic memory maintenance once. Call after a batch of memory_store writes when you need subsequent memory_record_search to immediately reflect them.",
        parameters: Type.Object({
        ...
        async execute(_toolCallId, params) {
            ...
            const { tenant_id, user_id } = resolveTenantUser(params);
            api.logger.info(`[memory_flush] timeoutMs=${timeoutMs ?? 60000} ` +
                `user=${formatScopedId("u", user_id)}`);
            const args = { userId: user_id };
            if (timeoutMs != null)
                args.timeoutMs = timeoutMs;
            const raw = (await client.callTool("memory_flush", args, 120_000));
            const ok = raw.status === 0;
            api.logger.info(`[memory_flush] → status=${raw.status} drained=${raw.drained} ` +
    ...
    api.registerTool({
        name: "memory_list",
        label: "Memory List",
        description: "List stored memories by semantic category: global overview, " +
        ...
        async execute(_toolCallId, params) {
            ...
            const { categories, layers, limit, offset, _userId } = params;
            const userId = _userId ?? cfg.userId;
            const layerMap = {
                global_overview: "l0",
                scene_memory: "l1",
            ...
            const internalLayers = Array.from(new Set(requestedCategories
            ...
            api.logger.info(`[memory_list] categories=${JSON.stringify(requestedCategories)} ` +
            ...
            if (offset != null)
                args.offset = offset;
            const raw = (await client.callTool("memory_list", args));
            const parts = [];
            if (raw.l0) {
                const l0 = raw.l0;
    ...
    api.registerTool({
        name: "memory_dump",
        label: "Memory Dump",
        description: "Export this session's memories as a JSON file under dumpDir. " +
            "Returns only the file path + record counts, never the payload itself.",
        parameters: Type.Object({
            sessionId: Type.Optional(Type.String({
                description: "Session ID. Omit to auto-derive from current user's tool session.",
            })),
            outputPath: Type.Optional(Type.String({
                description: "Relative path under dumpDir. Empty/omit = auto-name. " +
                    "Absolute paths or '..' are rejected.",
            })),
            category: Type.Optional(Type.Number({ description: "Filter by category (<0 = any)" })),
            sinceTimestampMs: Type.Optional(Type.Number({ description: "Lower bound timestamp in ms (0 = unbounded)" })),
            untilTimestampMs: Type.Optional(Type.Number({ description: "Upper bound timestamp in ms, exclusive (0 = unbounded)" })),
            timeField: Type.Optional(Type.String({
                description: "'updated_at_ms' (default) or 'created_at_ms'",
            })),
            includeSceneMemory: Type.Optional(Type.Boolean({ description: "Include scene memory summaries" })),
            includeGlobalOverview: Type.Optional(Type.Boolean({ description: "Include global overview" })),
        }),
        async execute(_toolCallId, params) {
            if (!client.isConnected() && !(await client.waitReady())) {
                return {
            ...
            if (p.outputPath != null)
                args.outputPath = p.outputPath;
            if (p.category != null)
                args.category = p.category;
            if (p.sinceTimestampMs != null)
                args.sinceTimestampMs = p.sinceTimestampMs;
            if (p.untilTimestampMs != null)
                args.untilTimestampMs = p.untilTimestampMs;
            if (p.timeField != null)
                args.timeField = p.timeField;
            const includeSceneMemory = p.includeSceneMemory ?? p.includeL1;
            const includeGlobalOverview = p.includeGlobalOverview ?? p.includeL0;
            if (includeSceneMemory != null)
                args.includeL1 = includeSceneMemory;
            if (includeGlobalOverview != null)
                args.includeL0 = includeGlobalOverview;
            api.logger.info(`[memory_dump] sessionId=${sessionId} userId=${userId} ` +
                `outputPath=${p.outputPath ?? "(auto)"} ` +
                `includeGlobalOverview=${includeGlobalOverview} ` +
                `includeSceneMemory=${includeSceneMemory}`);
            const raw = (await client.callTool("memory_dump", args, 120_000));
            const ok = raw.status === 0;
            const counts = raw.record_count ?? {};
            api.logger.info(`[memory_dump] → status=${raw.status} file=${raw.file_path} ` +
    ...
    api.registerTool({
        name: "dream_status",
        label: "Dream Status",
        description: "Query current/specified dream run progress (phase + percent). " +
        ...
        async execute(_toolCallId, params) {
            ...
            if (p.runId)
                args.runId = p.runId;
            const raw = (await client.callTool("dream_status", args));
            return raw;
        },
    }, { name: "dream_status" });
    api.registerTool({
        ...
        description: "Query the result summary of a finished dream run (emergent scenes, " +
        ...
        async execute(_toolCallId, params) {
            ...
            if (p.runId)
                args.runId = p.runId;
            const raw = (await client.callTool("dream_run_summary", args));
            return raw;
        },
    }, { name: "dream_run_summary" });
    api.registerTool({
        ...
        description: "List the most recent N dream runs with brief summaries. " +
        ...
        async execute(_toolCallId, params) {
            ...
            if (p.limit != null)
                args.limit = p.limit;
            const raw = (await client.callTool("dream_recent_runs", args));
            return raw;
        },
    }, { name: "dream_recent_runs" });
}
...
// CLI commands
// ============================================================================
export function registerCli(api, client, cfg) {

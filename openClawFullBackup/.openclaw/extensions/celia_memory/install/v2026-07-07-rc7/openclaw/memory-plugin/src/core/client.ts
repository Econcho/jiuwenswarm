...
// ============================================================================
// memory_load_l1 — batch wrapper types
// ============================================================================

/** memory_load_l1 batch 单条结果（与 C 端 mcp_tools_load_l1.c 输出对齐）。 */
export type LoadL1BatchItem = {
  path: string;
  status: "ok" | "not_found" | "invalid_path" | "parse_error";
  body?: string;
...
/** memory_load_l1 batch 响应聚合。 */
export type LoadL1BatchResponse = {
  results: LoadL1BatchItem[];
  totalBytes: number;
  trimmed: boolean;
};
...
export function parseLoadL1BatchResponse(raw: unknown): LoadL1BatchResponse {
  ...
  for (const r of rawResults) {
    ...
    results.push(item as unknown as LoadL1BatchItem);
  }
  const totalBytes =
    typeof obj.total_bytes === "number" ? obj.total_bytes : 0;
...
// ============================================================================
// B5：MCP 响应 _meta echo-back DFX
// ----------------------------------------------------------------------------
// C 端 mcp_tools_meta 在 search/load 工具响应顶层输出 `_meta`：
//   { trace_id, dur_ms, rerank_status?, scope_denied?, dedup_info? }
// 解析后通过 emitDFX("mcp.echo", ...) 发射到 TS 层 DFX 流，单条 trace_id
// 即可在 TS plugin / MCP / C 三层日志中 grep 串联。
// ============================================================================

interface ScopeDenied {
  tenant?: number;
  user?: number;
  session?: number;
  passed?: number;
...
interface DedupMeta {
  ...
  dropped_l1_child?: number;
  dropped_scene_overflow?: number;
}

interface McpRespMeta {
  trace_id?: string;
  dur_ms?: number;
  rerank_status?: string;
  scope_denied?: ScopeDenied;
  dedup_info?: DedupMeta;
}
 ...
 */
export function extractRespMeta(parsed: unknown): McpRespMeta | undefined {
  if (!parsed || typeof parsed !== "object") return undefined;
  const meta = (parsed as { _meta?: unknown })._meta;
  if (!meta || typeof meta !== "object") return undefined;
  return meta as McpRespMeta;
}

/** scope_denied 三个维度求和（任一字段缺失视为 0）。导出为测试可见。 */
export function sumScopeDenied(sd: ScopeDenied | undefined): number {
  if (!sd) return 0;
 ...
 */
function emitMcpEcho(toolName: string, parsed: unknown, traceId: string): void {
...
/** Decision returned by shouldRestartOnExit (exported for unit tests). */
export type RestartDecision =
  | { restart: true; reason: string }
  | { restart: false; reason: string };

export type BinaryExecutableCheck = {
  ok: boolean;
  repaired: boolean;
  reason?: string;
};

export function buildCeliaArgs(
  dbPath: string,
  logFilePath?: string,
): string[] {
...
const CELIA_TO_GSPD_ENV_ALIASES: Array<[celia: string, gspd: string]> = [
...
export function withGspdLegacyEnvAliases(
 ...
 */
export function shouldRestartOnExit(
  code: number | null,
 ...
 */
export function ensureExecutableBinary(binaryPath: string): BinaryExecutableCheck {
  if (!existsSync(binaryPath)) {
    return {
      ok: false,
      repaired: false,
      reason:
        `celia_memory_mcp_server binary not found: ${binaryPath}. ` +
  ...
  try {
    ...
    return {
      ...
      repaired: false,
      reason:
        `celia_memory_mcp_server binary is not executable and chmod repair failed: ` +
...
export class CeliaMcpClient {
   ...
   */
  private lastSpawnSucceededAtMs = 0;
  /**
  ...
  private async spawn(): Promise<void> {
    ...
    if (binaryCheck.repaired) {
      process.stderr.write(
        `[celia-mcp] repaired executable bit: ${this.binaryPath}\n`,
      );
    }

    this.proc = spawn(
      this.binaryPath,
      buildCeliaArgs(this.dbPath, this.logFilePath),
      {
    ...
    this.proc.unref();
    (this.proc.stdin as typeof this.proc.stdin & { unref?: () => void })?.unref?.();
    (this.proc.stdout as typeof this.proc.stdout & { unref?: () => void })?.unref?.();
    (this.proc.stderr as typeof this.proc.stderr & { unref?: () => void })?.unref?.();
    ...
    this.proc.on("error", (err) => {
      ...
      process.stderr.write(
        `[celia-mcp] spawn error: ${err.message}\n`,
      );
    ...
    this.proc.on("exit", (code, signal) => {
      ...
      if (!decision.restart) {
        process.stderr.write(
          `[celia-mcp] process ${decision.reason}, not restarting\n`,
        );
        return;
      }
      ...
      if (this.restartAttempts >= RESTART_MAX_CONSECUTIVE_ATTEMPTS) {
        process.stderr.write(
          `[celia-mcp] giving up after ${RESTART_MAX_CONSECUTIVE_ATTEMPTS} ` +
      ...
      process.stderr.write(
        `[celia-mcp] process ${decision.reason} (code=${code} signal=${signal}); ` +
  ...
  async callTool(
    ...
    const augmentedArgs: Record<string, unknown> =
      existing ? args : { ...args, _trace_id: traceId };

    if (DEBUG_TOOLS.has(name)) {
      const argsPreview = JSON.stringify(augmentedArgs).slice(0, 200);
      console.info(
        `memory-celia: [MCP→] trace_id=${traceId} tool=${name} args=${argsPreview}`,
      );
    }
    ...
    if (result.isError) {
      ...
      if (DEBUG_TOOLS.has(name)) {
        console.info(
          `memory-celia: [MCP←] tool=${name} error="${text.slice(0, 200)}"`,
        );
      }
    ...
    if (DEBUG_TOOLS.has(name)) {
      ...
      console.info(
        `memory-celia: [MCP←] tool=${name} resp=${respPreview}`,
      );
    }
  ...
  ): Promise<LoadL1BatchResponse> {
    if (!Array.isArray(paths) || paths.length === 0) {
      return { results: [], totalBytes: 0, trimmed: false };
    }
    const args: Record<string, unknown> = { paths };
    if (opts?.tenantId) args.tenant_id = opts.tenantId;
  ...
  async waitReady(timeoutMs = 5000): Promise<boolean> {
    if (this.isConnected() && !this.startPromise) return true;

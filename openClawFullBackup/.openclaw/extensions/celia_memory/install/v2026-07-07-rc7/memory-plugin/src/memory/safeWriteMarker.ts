...
import {
    type MarkerName,
    hasLegacyMarkerBlocks,
    parseMarker,
    removeMarkerBlocks,
    replaceOrInjectMarker,
    sha256Short,
    truncateUtf8Readable,
    truncateUtf8Safe,
    utf8ByteLength,
} from "./markerProtocol.js";
import { emitDFX } from "../core/dfx.js";

// ---- 字符预算常量 -----------------------------------------------------------

/** 各 marker 的硬上限字节数（详见 spec §6.4.3）。 */
export const MARKER_BUDGETS: Record<MarkerName, number> = {
...
const MEMORY_ACTION_LOG_PATH =
    "/home/sandbox/.openclaw/logs/celia_memory/celia_memory.log";
...
/**
 * L1 摘要条目。来自 C 层 `memory_get_l1_index` 的单条 entry。
 *
 * 字段命名与 C 层对齐（`L1IndexEntry`）。
 */
...
export interface TrimReport {
    dropped: string[]; /* 被丢弃的条目 id 或区块名（L0 是 "B"/"C" 等） */
    reason: string;
    perSummaryCap?: number; /* L1_SCENES 等比缩短时的实际 cap（字节） */
}

/** 与 OpenClaw api.logger 兼容的简化 logger 接口。 */
export interface SafeWriteLogger {
    info?: (msg: string) => void;
    warn?: (msg: string) => void;
}

/** safeWriteMarker 调用上下文。 */
export interface SafeWriteCtx {
    traceId: string;
    logger?: SafeWriteLogger;
    /** 测试可覆盖 `.openclaw/.memory.log` 路径；null 表示禁用。 */
    memoryChangeLogPath?: string | null;
    /** 测试可覆盖 `logs/celia_memory/celia_memory.log` 路径；null 表示禁用。 */
    memoryActionLogPath?: string | null;
}

/** safeWriteMarkers 批量补丁项。 */
export interface MarkerPatch {
    markerName: MarkerName;
    inputContent: string | LtmSummary[];
}
...
function formatLocalTimestamp(now = new Date()): string {
    return `${now.getFullYear()}-${pad2(now.getMonth() + 1)}-` +
        `${pad2(now.getDate())}T${pad2(now.getHours())}:` +
        `${pad2(now.getMinutes())}:${pad2(now.getSeconds())}`;
}

function memoryChangeDesc(markerName: MarkerName): string | null {
...
function shouldCleanMigratedOverview(
    filePath: string,
...
function cleanupMigratedOverview(
    filePath: string,
    content: string,
    markerNames: MarkerName[],
): MdFilePreparation {
...
function prepareMdFileForWrite(
    filePath: string,
    content: string,
    markerNames: MarkerName[],
): MdFilePreparation {
    const cleanup = cleanupMigratedOverview(filePath, content, markerNames);
    const normalized = normalizeManagedMarkerTails(
        filePath,
        cleanup.content,
        markerNames,
    );
...
function shouldKeepMarkerAtFileTail(
    filePath: string,
    markerName: MarkerName,
): boolean {
...
function trimMovedTailContent(content: string): string {
    return content
        .replace(/^(?:[ \t]*\r?\n)+/, "")
        .replace(/[ \t\r\n]+$/, "");
}

function assembleMarkerTailNormalization(
    before: string,
    movedTail: string,
    markerBlock: string,
): string {
...
function normalizeManagedMarkerTail(
    filePath: string,
    content: string,
    markerName: MarkerName,
): MdFilePreparation {
    ...
    return {
        content: assembleMarkerTailNormalization(
            before,
            movedTail,
            markerBlock,
        ),
...
function normalizeManagedMarkerTails(
    filePath: string,
    content: string,
    markerNames: MarkerName[],
): MdFilePreparation {
    ...
    for (const markerName of markerNames) {
        const normalized = normalizeManagedMarkerTail(
            filePath,
            current,
            markerName,
        );
...
async function appendMemoryChangeDescriptions(
    filePath: string,
    descriptions: string[],
    ctx: SafeWriteCtx,
): Promise<void> {
    ...
    const logPath = resolveMemoryLogPath(
        filePath,
        ctx.memoryChangeLogPath,
        MEMORY_CHANGE_LOG_PATH,
    );
    if (logPath === null) {
        return;
    }
...
async function appendLogLines(
    ...
    try {
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarker] WARN ${logKind}_failed ` +
...
function appendContentChangeDescriptions(
    filePath: string,
    descriptions: string[],
    reason: string,
    ctx: SafeWriteCtx,
...
function markerActionName(
    action: MarkerWriteResult["action"],
    reason: string,
): string {
...
function markerActionDescription(
    markerName: MarkerName,
    action: MarkerWriteResult["action"],
    reason: string,
    bytes: number,
...
function fileActionDescription(action: string, reason: string): string {
    return `action=file.${action} reason=${reason}`;
}

async function appendMemoryActionDescriptions(
    filePath: string,
    descriptions: string[],
    ctx: SafeWriteCtx,
): Promise<void> {
    ...
    const logPath = resolveMemoryLogPath(
        filePath,
        ctx.memoryActionLogPath,
        MEMORY_ACTION_LOG_PATH,
    );
    if (logPath === null) {
        return;
    }
...
function isPlaceholderSemanticLine(line: string): boolean {
...
function normalizeMarkerSemanticBody(body: string): string {
    return body
        .replace(/\r\n/g, "\n")
        .split("\n")
        .map((line) => line.trim())
...
export function hasMarkerSemanticContent(body: string): boolean {
    return normalizeMarkerSemanticBody(body).length > 0;
}

function markerSemanticChanged(oldBody: string, newBody: string): boolean {
...
type OverviewLogSection =
    | "constraints"
    | "profile"
    | "preferences"
    | "intents";

const OVERVIEW_LOG_SECTION_ORDER: OverviewLogSection[] = [
    "constraints",
    "profile",
    "preferences",
    "intents",
...
const OVERVIEW_LOG_SECTION_DESCS: Record<OverviewLogSection, string> = {
...
const OVERVIEW_LOG_SECTION_TITLES: Record<OverviewLogSection, string[]> = {
    constraints: [
...
function overviewHeadingSection(line: string): OverviewLogSection | null {
...
function overviewLineSection(line: string): OverviewLogSection | null {
...
function extractOverviewLogSections(
    body: string,
): Record<OverviewLogSection, string> {
    const sections: Record<OverviewLogSection, string> = {
        constraints: "",
        profile: "",
        preferences: "",
        intents: "",
    };
    let currentSection: OverviewLogSection | null = null;
...
function overviewChangeDescriptions(
    oldBody: string,
    newBody: string,
): string[] {
...
function markerChangeDescriptions(
    markerName: MarkerName,
    oldBody: string,
    newBody: string,
): string[] {
...
export function renderContent(
    name: MarkerName,
    content: string | LtmSummary[],
    /* 可选：每条 summary 字节上限（等比缩短时使用） */
    perSummaryCap?: number,
): string {
    if (typeof content === "string") return content;

    if (name !== "MEMORY_SCENES") {
        throw new Error(
            `renderContent: 仅 MEMORY_SCENES 接受 LtmSummary[]（实际 ${name}）`,
        );
    }
    ...
    const lines: string[] = ["# Celia Scenario Memory Summaries", ""];
...
export interface TrimResult {
    /** 截断后的 marker 内容（即将写入 marker 区的 markdown） */
    trimmedContent: string;
    /** 被丢弃的条目（id 或区块名） */
    dropped: string[];
    /** L1_SCENES 等比缩短时的实际 cap（字节） */
    perSummaryCap?: number;
}
...
export function trimByPolicy(
    ...
    if (name === "MEMORY_GUIDE") {
        ...
        if (utf8ByteLength(content) > budget) {
            throw new Error(
                `MEMORY_GUIDE 模板字节数 ${utf8ByteLength(content)} ` +
 ...
 */
function trimL0ByPriority(content: string, eb: number): TrimResult {
    ...
    const trimOrder: Array<"C" | "B" | "D" | "A"> = ["C", "B", "D", "A"];
    for (const blkName of trimOrder) {
        let assembled = assembleL0Blocks(blocks);
        if (utf8ByteLength(assembled) <= eb) break;
        const blk = blocks[blkName];
        if (!blk) continue;
        if (blkName === "A" || blkName === "D") {
            /* 必保区：仅尾截到当前剩余预算 */
            const otherBytes =
                utf8ByteLength(assembled) - utf8ByteLength(blk);
...
function splitL0Blocks(content: string): Record<string, string> {
    const out: Record<string, string> = { _: "", A: "", B: "", C: "", D: "" };
    const lines = content.split("\n");
    let cur: keyof typeof out = "_";
...
function assembleL0Blocks(blocks: Record<string, string>): string {
    return ["_", "A", "B", "C", "D"]
        .map((k) => blocks[k])
...
function trimL0ListBlock(
    ...
    while (items.length > 0) {
        const trial = assembleL0ListWithTruncation(
            header,
            items,
            hasTruncatedItems,
        );
        const tryAssembled =
            assembled.replace(block, trial); /* 估算 */
    ...
    const fallback = assembleL0ListWithTruncation(
        header,
        items,
        hasTruncatedItems,
    );
...
function assembleL0ListWithTruncation(
    header: string,
    items: string[],
    hasTruncatedItems: boolean,
): string {
    const out: string[] = [];
    if (header) {
        out.push(header);
...
/** L1 场景：等比缩短保 ALL。 */
function trimL1ScenesEqualShrink(
    scenes: LtmSummary[],
    eb: number,
): TrimResult {
    ...
    const perCap = Math.max(
        0,
        Math.min(PER_SUMMARY_HARD_CAP, Math.floor(summariesBudget / N)),
    );
...
async function lockFile(
    filePath: string,
    opts: LockOptions = { retries: 5, staleMs: 5000 },
    ...
    throw new Error(
        `lockFile failed after ${opts.retries} retries: ${filePath} ` +
            `(${String(lastErr)})`,
    );
}
 ...
 * @returns 大写文件的绝对路径
 */
export async function ensureUppercaseMd(
    workspaceDir: string,
    name: "USER.md" | "MEMORY.md" | "AGENTS.md",
    ctx: SafeWriteCtx,
): Promise<string> {
    ...
    if (upperExists) return upper;

    const lowerExists = await fileExists(lower);
    if (lowerExists) {
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarker] WARN ` +
    ...
    await fsp.writeFile(upper, "", { flag: "wx" }).then(() => {
        ...
        const e = err as NodeJS.ErrnoException;
        if (e.code === "EEXIST") return; /* 并发竞争：他人已创建 */
        throw err;
    });
    if (created) {
        await appendMemoryActionDescriptions(
            upper,
            [fileActionDescription("create", "ensure_uppercase")],
            ctx,
        );
    }
    return upper;
 ...
 */
export async function safeWriteMarker(
    filePath: string,
    markerName: MarkerName,
    inputContent: string | LtmSummary[],
    ctx: SafeWriteCtx,
    ...
    let trimmedReport: TrimReport | undefined;
    try {
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarker] WARN render_error ` +
    ...
    try {
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarker] WARN lock_failed ` +
    ...
    try {
        ...
        const preparation =
            prepareMdFileForWrite(filePath, existing, [markerName]);
        ...
        const claimedActualHash = parsed.primary
            ...
            : "";
        const markerTampered =
            parsed.primary !== null && claimedActualHash !== oldHash;
        if (markerTampered) {
        ...
        const emptyMerged = hasSemanticContent
            ? ""
            : normalizeManagedMarkerTails(
                filePath,
                emptyRawMerged,
                [markerName],
            ).content;
        const emptyCleanupChanged =
            !hasSemanticContent && emptyMerged !== existing;
        ...
        const semanticMerged = hasSemanticContent
            ...
            : "";
        const semanticCleanupChanged =
            hasSemanticContent && semanticMerged !== existing;
        if (
            !hasSemanticContent &&
            parsed.primary === null &&
            parsed.orphans.length === 0 &&
        ...
        const semanticChanged = hasSemanticContent
            ? markerSemanticChanged(
                parsed.primary?.content ?? "",
                renderedContent,
            )
            : false;
        const changeDescriptions = hasSemanticContent
            ? markerChangeDescriptions(
                markerName,
                parsed.primary?.content ?? "",
                renderedContent,
            )
        ...
        if (parsed.broken && originalExisting.length > 0) {
            const rescuePath =
                `${filePath}.celia-rescue.${Date.now()}`;
            try {
                ...
                ctx.logger?.warn?.(
                    `memory-celia: [safeWriteMarker] rescue ` +
                        `failed: ${String(err)}`,
                );
        ...
        if (totalBytes > FILE_TOTAL_LIMIT) {
            ctx.logger?.warn?.(
                `memory-celia: [safeWriteMarker] WARN total_over_limit ` +
                    `file=${path.basename(filePath)} ` +
        ...
        const baseAction: "create" | "update" =
            hasSemanticContent && !parsed.primary ? "create" : "update";
        const action: MarkerWriteResult["action"] = trimmedReport
        ...
        const reason = markerActionReason(
            hasSemanticContent,
            !hasSemanticContent,
            semanticChanged,
            changeDescriptions,
        );
        await appendContentChangeDescriptions(
            filePath,
            changeDescriptions,
            reason,
            ctx,
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarker] WARN write_failed ` +
                `file=${filePath} err=${String(err)}`,
        );
...
// ---- 批量入口 safeWriteMarkers ----------------------------------------------

/**
 * 单 patch 的"渲染→预算自校验"中间结果，供 safeWriteMarkers 内部传递。
 */
interface RenderedPatch {
    markerName: MarkerName;
    /** 渲染并裁剪后的 marker body（即将写入 marker 区） */
    body: string;
    bytes: number;
    budget: number;
    newHash: string;
    /** 渲染失败时为 null：本 patch 整体退化为 noop */
    failed: boolean;
    /** marker body 是否包含标题/占位符之外的用户可见语义。 */
    hasSemanticContent: boolean;
    trimmedReport?: TrimReport;
}
...
function renderPatchForBatch(
    ...
    try {
        ...
        if (utf8ByteLength(rendered) <= budget) {
            ...
            if (
                patch.markerName === "MEMORY_SCENES" &&
                trim.perSummaryCap !== undefined &&
                trim.perSummaryCap < PER_SUMMARY_SOFT_FLOOR
            ) {
                ctx.logger?.warn?.(
                    `memory-celia: [safeWriteMarkers] WARN ` +
                        `scene_summary_starvation perCap=` +
                        `${trim.perSummaryCap}B < ${PER_SUMMARY_SOFT_FLOOR}B`,
                );
                emitDFX(ctx, "fixedLoad.scene_starvation", {
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarkers] WARN render_error ` +
...
/** 单个 patch 的写入决策（对应 batch 主流程内的状态）。 */
interface PatchDecision {
    /** 是否需要 replaceOrInjectMarker（false 表示沿用 existing 中已有内容） */
    mustWrite: boolean;
    /** 现有 marker 是否存在（决定 action=create vs update） */
    hadPrimary: boolean;
    /** marker body 中的用户可见语义是否发生变化。 */
    semanticChanged: boolean;
    /** 本次写入对应的用户可见语义变化描述。 */
    changeDescriptions: string[];
    oldHash: string;
    /** true 表示新内容为空语义，本次应移除已有 marker 块。 */
    removeMarker: boolean;
}
...
function decidePatchWrite(
    ...
    const emptyCleanupChanged =
        !rendered.hasSemanticContent && emptyMerged !== existing;
    ...
    const semanticMerged = rendered.hasSemanticContent
        ...
        : "";
    const semanticCleanupChanged =
        rendered.hasSemanticContent && semanticMerged !== existing;
    if (
        !rendered.hasSemanticContent &&
        parsed.primary === null &&
        parsed.orphans.length === 0 &&
        !parsed.broken &&
        !emptyCleanupChanged
    ...
    return {
        ...
        hadPrimary: parsed.primary !== null,
        semanticChanged,
        changeDescriptions: markerChangeDescriptions(
            rendered.markerName,
            oldBody,
            rendered.body,
        ),
 ...
 */
export async function safeWriteMarkers(
    filePath: string,
    patches: MarkerPatch[],
    ctx: SafeWriteCtx,
): Promise<MarkerWriteResult[]> {
    ...
    const results: MarkerWriteResult[] = rendered.map((r) => ({
    ...
    try {
        ...
        ctx.logger?.warn?.(
            `memory-celia: [safeWriteMarkers] WARN lock_failed ` +
                `file=${filePath} err=${String(err)}`,
        );
        emitDFX(ctx, "fixedLoad.lock_failed", {
            file: path.basename(filePath),
    ...
    try {
        ...
        const patchMarkers = rendered
            ...
            .map((r) => r.markerName);
        const preparation =
            prepareMdFileForWrite(filePath, existing, patchMarkers);
        existing = preparation.content;

        /* ========== 阶段 5：逐 patch 决策 ========== */
        const decisions: PatchDecision[] = rendered.map((r) =>
            r.failed
                ? {
                    mustWrite: false,
                    hadPrimary: false,
        ...
        if (preparation.changed) {
            const cleanupIndex = decisions.findIndex((d, i) =>
                !rendered[i].failed &&
        ...
        const brokenSeen = rendered.some(
            (r) => !r.failed && parseMarker(existing, r.markerName).broken,
        );
        if (brokenSeen && originalExisting.length > 0) {
            ...
            try {
                ...
                ctx.logger?.warn?.(
                    `memory-celia: [safeWriteMarkers] rescue ` +
                        `failed: ${String(err)}`,
                );
        ...
        for (let i = 0; i < rendered.length; i++) {
            const r = rendered[i];
            const d = decisions[i];
            if (r.failed || !d.mustWrite) continue;
        ...
        merged = normalizeManagedMarkerTails(
            filePath,
            merged,
            patchMarkers,
        ).content;
        ...
        if (totalBytes > FILE_TOTAL_LIMIT) {
            ctx.logger?.warn?.(
                `memory-celia: [safeWriteMarkers] WARN total_over_limit ` +
                    `file=${path.basename(filePath)} ` +
                    `total=${totalBytes} > ${FILE_TOTAL_LIMIT}`,
            );
            emitDFX(ctx, "fixedLoad.warn_total_over_limit", {
        ...
        try {
            ...
            ctx.logger?.warn?.(
                `memory-celia: [safeWriteMarkers] WARN write_failed ` +
                    `file=${filePath} err=${String(err)}`,
            );
            emitDFX(ctx, "fixedLoad.write_failed", {
                file: path.basename(filePath),
        ...
        for (let i = 0; i < rendered.length; i++) {
            ...
            const baseAction: "create" | "update" =
                (d.hadPrimary || d.removeMarker) ? "update" : "create";
            const action: MarkerWriteResult["action"] = r.trimmedReport
                ? baseAction === "create"
                    ? "trim+create"
                    : "trim+update"
                : baseAction;

...
const CURRENT_MARKER_WIRE_NAMES: Record<MarkerName, string> = {
 ...
 * marker BEGIN 行的元信息。
 *
 * BEGIN 行仅支持 `h=<short>` 字段。
 */
export interface MarkerMeta {
    /** 内容指纹（短 hex，长度 = MARKER_HASH_LEN）。 */
    hash: string;
}

/** 单个解析结果（含 marker 在文件中的字节范围）。 */
export interface ParsedMarker {
    name: MarkerName;
    meta: MarkerMeta;
    content: string; /* BEGIN 与 END 之间的内容（不含两行 marker 本身） */
    range: { start: number; end: number }; /* 含 BEGIN 行起、END 行止的字符偏移 */
}

/** parseMarker 返回结构。 */
export interface ParseResult {
    primary: ParsedMarker | null;
    orphans: ParsedMarker[];
    broken: boolean; /* 找到 BEGIN 但找不到对应 END */
}
...
function sliceCodePoints(s: string, count: number): string {
...
function findLastNaturalBoundary(
    prefix: string,
 ...
 */
export function truncateUtf8Readable(s: string, maxBytes: number): string {
    if (utf8ByteLength(s) <= maxBytes) return s;
...
/** BEGIN 行整体匹配，多行模式。 */
const MARKER_BEGIN_RE =
    /<!--\s+([A-Z0-9_]+)_BEGIN\s+h=([^\s>]+)\s*-->/g;
...
export function hasLegacyMarkerBlocks(fileContent: string): boolean {
    for (const name of LEGACY_MARKER_NAMES) {
        const markerLineRe = new RegExp(
            `<!--\\s+${name}_(?:BEGIN|END)[^>]*-->`,
        );
        if (markerLineRe.test(fileContent)) return true;
    }
    return false;
}

function removeNamedMarkerBlocks(fileContent: string, name: string): string {
...
function unwrapNamedMarkerLines(fileContent: string, name: string): string {
    const escapedName = escapeRegExp(name);
    const markerLineRe = new RegExp(
        `^[\\t ]*<!--\\s+${escapedName}_(?:BEGIN|END)[^>]*-->` +
            `[\\t ]*(?:\\r?\\n|$)`,
        "gm",
    );
...
function unwrapNamedMarkerBlocks(fileContent: string, name: string): string {
    const escapedName = escapeRegExp(name);
    const blockRe = new RegExp(
        `^[\\t ]*<!--\\s+${escapedName}_BEGIN[^>]*-->` +
            `[\\t ]*(?:\\r?\\n|$)` +
            `([\\s\\S]*?)` +
            `^[\\t ]*<!--\\s+${escapedName}_END[^>]*-->` +
            `[\\t ]*(?:\\r?\\n|$)`,
    ...
    const unwrapped = fileContent.replace(
        blockRe,
        (_match: string, body: string) => body.trim() === "" ? "" : body,
    );
    return unwrapNamedMarkerLines(unwrapped, name);
}

function removeLegacyMarkerBlocks(fileContent: string): string {
    let cleaned = fileContent;
    for (const name of LEGACY_MARKER_NAMES) {
...
function normalizeBlankOnlyContent(fileContent: string): string {
    return fileContent.length > 0 && fileContent.trim() === ""
        ? ""
        : fileContent;
}

function normalizeBeforeReplaceOrInject(fileContent: string): string {
...
export function removeMarkerBlocks(
    fileContent: string,
    markerName: MarkerName,
): string {
    return removeNamedMarkerBlocks(
        fileContent,
        CURRENT_MARKER_WIRE_NAMES[markerName],
    );
}
 ...
 */
export function unwrapLegacyMemoryMarkers(fileContent: string): string {
    let cleaned = fileContent;
...
export function parseMarker(
    ...
    const completedPairs: ParsedMarker[] = [];
    let pendingBegin: Match | null = null;
    for (const mm of matches) {
        if (mm.kind === "begin") {
            ...
            if (pendingBegin === null) continue; /* 孤立 END，忽略 */
    ...
    if (pendingBegin !== null) result.broken = true;
...
function buildBeginLine(name: MarkerName, hash: string): string {
    return `<!-- ${CURRENT_MARKER_WIRE_NAMES[name]}_BEGIN h=${hash} -->`;
}

/** 构造 END 行。 */
function buildEndLine(name: MarkerName): string {
 ...
 * @returns            替换后的完整文件内容
 */
export function replaceOrInjectMarker(
    fileContent: string,
    markerName: MarkerName,
    newContent: string,
    newHash: string,
 ...
 */
function removeOrphanBeginEndLines(
    fileContent: string,
    markerName: MarkerName,
): string {
    const re = new RegExp(
        `<!--\\s+${CURRENT_MARKER_WIRE_NAMES[markerName]}_` +
            `(?:BEGIN|END)[^>]*-->\\n?`,
        "g",
    );

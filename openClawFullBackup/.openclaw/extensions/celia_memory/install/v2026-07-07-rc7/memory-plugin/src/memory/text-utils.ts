 ...
 * 末尾 dashes 数量不固定也兼容。
 */
const OPENCLAW_SKILL_LIST_PREFIX_RE =
  /##\s*用户查询相关skill列表如下：[\s\S]*?---\s*以下是用户原始[^\n-]*-+\s*\n*/;
...
const LEADING_OPENCLAW_TIMESTAMP_RE =
  /^\s*\[[A-Z][a-z]{2}\s+\d{4}-\d{2}-\d{2}\s+\d{1,2}:\d{2}\s+GMT[+-]\d{1,2}\]\s*/;

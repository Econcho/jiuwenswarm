 ...
 */
export function sanitizeConversationRound(messages, startIdx) {
    const result = [];
    for (let i = startIdx; i < messages.length; i++) {
        ...
        if (!raw)
            continue; /* 纯 tool call 的 assistant 消息，整条丢弃 */
        if (role === "user") {
 ...
 */
export function formatCleanedRound(messages) {
    return messages

...
import { safeWriteMarker, } from "./safeWriteMarker.js";
import { extractL1Entries, } from "./fixedContext.js";
 ...
 * 各自发起重复的 MCP 请求。
 */
export async function fetchL1Index(client, tenantId, userId, ctx) {
    const ms = ctx.deadlineMs ?? L1_INDEX_FETCH_DEADLINE_MS;
    const traceArgs = ctx.traceId
    ...
    const raw = await Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error("L1 index fetch deadline exceeded")), ms)),
    ]);
 ...
 */
export function filterL1ByType(entries, typeFilter) {
    return entries
 ...
 */
export async function fetchL1ByType(client, tenantId, userId, typeFilter, ctx) {
 ...
 */
export function selectFixedLoadScenes(scenes) {
...
export async function syncL1ScenesToMemoryMd(client, workspaceDir, tenantId, userId, ctx, prefetched) {
    let scenes;
    if (prefetched) {
        scenes = prefetched;
    }
        ...
        try {
            scenes = await fetchL1ByType(client, tenantId, userId, "SCENE", ctx);
        }
        catch (err) {
            ctx.logger?.warn?.(`memory-celia: [l1ScenesSync] trace=${ctx.traceId} ` +
    ...
    return await safeWriteMarker(targetPath, "MEMORY_SCENES", scenes, ctx);
}

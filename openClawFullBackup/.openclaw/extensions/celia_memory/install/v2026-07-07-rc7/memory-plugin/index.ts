...
import {
  readBorrowedConfig,
  resolveBorrowedChatProvider,
} from "./src/core/borrowed-config.js";
import {
  deriveCeliaLogFilePath,
  ensureCeliaLogDir,
} from "./src/core/log-path.js";
import {
  createLazyCeliaClientProxy,
  getOrCreateCeliaClient,
  type CeliaMcpClient,
} from "../shared/celia-client-singleton.js";
...
import {
  registerMdFileSync,
  registerConversationCapture,
  registerServedL1Cleanup,
  registerTokenStatsCapture,
  registerGatewayObserver,
...
export default definePluginEntry({
  ...
  name: "Celia Memory",
  description:
    "Four semantic memory layers with original conversations, atomic facts, " +
  ...
  register(api: OpenClawPluginApi) {
     ...
     */
    const dreamCfg = (cfg as { dreamingEnabled?: boolean | string | number })
      .dreamingEnabled;
    ...
    if (!envBuild.embed.apiKey || !envBuild.embed.baseUrl) {
      api.logger.warn(
        "celia: embed not configured (memory-celia.config.embed + " +
    ...
    if (!envBuild.chat.apiKey || !envBuild.chat.baseUrl) {
      api.logger.warn(
        "celia: chat not configured (memory-celia.config.chat + " +
    ...
    try {
      ...
      api.logger.warn(
        `celia: failed to create log dir for ${logFilePath}: ${String(err)}`,
      );
    }
    const client = getOrCreateCeliaClient(
    ...
    void client.start()
      .then(() => {
        api.logger.info(
          `celia: server prewarmed (binary: ${cfg.serverBinaryPath}, db: ${resolvedDbPath})`,
        );
      })
      .catch((err) => {
    ...
    api.logger.info(
      `celia: registered (binary: ${cfg.serverBinaryPath}, db: ${resolvedDbPath})`,
    );

 ...
 */
export function getOrCreateCeliaClient(
  binaryPath: string,
  dbPath: string,
  env: Record<string, string>,
  logFilePath?: string,
): CeliaMcpClient {
  const g = globalThis as typeof globalThis & { [CELIA_CLIENT_KEY]?: CeliaClientState };
...
export function getExistingCeliaClient(): CeliaMcpClient | undefined {
  const g = globalThis as typeof globalThis & { [CELIA_CLIENT_KEY]?: CeliaClientState };
  return g[CELIA_CLIENT_KEY]?.client;
}
 ...
 */
export function createLazyCeliaClientProxy(): CeliaMcpClient {
  const requireClient = (): CeliaMcpClient => {
  ...
  return {
    ...
    callTool: (
      ...
      timeoutMs?: number,
      opts?: { traceId?: string },
    ) => requireClient().callTool(name, args, timeoutMs, opts),
    loadL1Batch: (
      paths: string[],
      opts?: {
        traceId?: string;
    ...
    ) => requireClient().loadL1Batch(paths, opts),
    close: () => {
      /* Discovery proxy does not own the singleton ref-count. */
    },
    isConnected: () => getExistingCeliaClient()?.isConnected() ?? false,
    waitReady: (timeoutMs?: number) =>
      getExistingCeliaClient()?.waitReady(timeoutMs) ?? Promise.resolve(false),
  } as unknown as CeliaMcpClient;
}
 ...
 */
export function releaseCeliaClient(): void {
  const g = globalThis as typeof globalThis & { [CELIA_CLIENT_KEY]?: CeliaClientState };
  if (!g[CELIA_CLIENT_KEY]) return;
  g[CELIA_CLIENT_KEY].refCount--;
...
export function getSharedActiveSessionsMap(): Map<string, ActiveSessionState> {
  const g = globalThis as typeof globalThis & {
    [CELIA_SESSIONS_KEY]?: Map<string, ActiveSessionState>;
  };

 ...
 */
function getCeliaEnvPath(): string {
...
export function loadCeliaEnv(): void {
  if (!loaded) {
    ...
    try {
      ...
      if (code === "ENOENT") {
        console.warn(
          `[celia] .xiaoyienv not found at ${envPath}, skipping`,
        );
      } else {
        console.warn(
          `[celia] .xiaoyienv read failed at ${envPath} ` +
            `(${code ?? "unknown"}): ${(err as Error).message}, skipping`,
        );
      }
    }
 ...
 */
export function _resetCeliaEnvLoadedForTest(): void {
  loaded = false;
}
 ...
 * 不在此重复。
 */
export function aliasCeliaChatEnv(): void {
  if (!process.env.SERVICE_URL) return;

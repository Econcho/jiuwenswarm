...
视频生成完成后，**必须**调用 `aigc_marker` 技能为生成的视频文件添加 AIGC 隐式标识（仅元数据，不重编码，不改变画面）。

```bash
cd <aigc_marker技能路径>/scripts && python main.py <视频文件路径> --skip-visible
```

说明：
- `--skip-visible` 表示仅添加隐式元数据（AIGC 签名），不添加显式画面水印，不触发 ffmpeg 重编码，速度快且不改变视频画质
- 隐式水印写入视频元数据，符合 AIGC 标识规范
- 如果 aigc_marker 调用失败，**不阻塞流程**，记录警告后继续下一步

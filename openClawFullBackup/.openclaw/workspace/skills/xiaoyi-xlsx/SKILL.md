...
name: xiaoyi-xlsx
description: >
   电子表格综合处理技能，涵盖 .xlsx/.xlsm/.csv/.tsv 的创建、编辑与数据分析。
   触发条件：
   1. 用户上传了 .xlsx/.xlsm/.csv/.tsv 文件——无条件触发；
   2. 意图包含做表、整理、汇总、分析、对比数据、建预算、建财务模型；
   3. 涉及公式（VLOOKUP/SUMIFS等）、数据透视表、条件格式、跨表引用；
   4. 任务的自然输出为表格格式。不适用：主要交付物为 Word 文档、HTML 报告或独立脚本，即使涉及表格数据也不触发。
---
